...
const Dashboard: React.FC<DashboardProps> = ({ players, isAdmin }) => {
  ...
  if (!isAdmin) {
    const spotsLeft = Math.max(0, 20 - accepted);

    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] animate-fade-in space-y-8">
        <div className="text-center">
          <h1 className="text-3xl font-bold text-slate-800 mb-2">Upcoming Skate</h1>
          <p className="text-slate-500">Here's the current headcount.</p>
        </div>

        <div className="bg-white p-10 rounded-3xl shadow-xl border border-slate-100 text-center transform hover:scale-105 transition-transform duration-300">
          <div className="mb-4 inline-flex p-4 bg-green-50 rounded-full text-green-600">
            <CheckCircle className="w-10 h-10" />
          </div>
          <div className="text-7xl font-black text-slate-900 mb-2 tracking-tight">{accepted}</div>
          <p className="text-lg text-slate-500 font-bold uppercase tracking-widest">Skaters Confirmed</p>

          <div className="mt-8 pt-6 border-t border-slate-100 flex justify-center gap-8 text-sm text-slate-400">
            <span>Goalies: <strong className="text-slate-700">{goaliesIn}</strong></span>
            <span>Spots Left: <strong className="text-slate-700">{spotsLeft}</strong></span>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <div className="bg-white p-6 rounded-2xl shadow-lg border border-slate-100 hover:shadow-xl hover:-translate-y-1 transition-all duration-300 flex items-center space-x-4 group">
          <div className="p-4 bg-gradient-to-br from-blue-50 to-blue-100 rounded-2xl text-blue-600 group-hover:scale-110 transition-transform duration-300 shadow-inner">
        ...
        <div className="bg-white p-6 rounded-2xl shadow-lg border border-slate-100 hover:shadow-xl hover:-translate-y-1 transition-all duration-300 flex items-center space-x-4 group">
          <div className="p-4 bg-gradient-to-br from-green-50 to-green-100 rounded-2xl text-green-600 group-hover:scale-110 transition-transform duration-300 shadow-inner">
        ...
        <div className="bg-white p-6 rounded-2xl shadow-lg border border-slate-100 hover:shadow-xl hover:-translate-y-1 transition-all duration-300 flex items-center space-x-4 group">
          <div className="p-4 bg-gradient-to-br from-emerald-50 to-emerald-100 rounded-2xl text-emerald-600 group-hover:scale-110 transition-transform duration-300 shadow-inner">
        ...
        <div className="bg-white p-6 rounded-2xl shadow-lg border border-slate-100 hover:shadow-xl hover:-translate-y-1 transition-all duration-300 flex items-center space-x-4 group">
          <div className="p-4 bg-gradient-to-br from-purple-50 to-purple-100 rounded-2xl text-purple-600 group-hover:scale-110 transition-transform duration-300 shadow-inner">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        {/* RSVP Chart */}
        <div className="bg-white p-6 rounded-3xl shadow-lg border border-slate-100 h-96 hover:shadow-xl transition-shadow duration-300">
        ...
        {/* Goalie Check */}
        <div className="bg-white p-6 rounded-3xl shadow-lg border border-slate-100 hover:shadow-xl transition-shadow duration-300 flex flex-col justify-center">
          ...
          <div className="space-y-4">
            {goalies.map(g => (
              <div key={g.id} className="flex items-center justify-between p-4 bg-slate-50 rounded-lg">
                ...
                <span className={`px-3 py-1 rounded-full text-xs font-bold ${g.status === RsvpStatus.ACCEPTED ? 'bg-green-100 text-green-700' :
