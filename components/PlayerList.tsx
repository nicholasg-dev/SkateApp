  ...
  if (!isAdmin) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[400px] text-slate-400 animate-fade-in">
        ...
        <p>You need administrator privileges to view the roster.</p>
      </div>
    );
  }
          ...
          <table className="w-full">
            <thead className="bg-slate-50/80 backdrop-blur border-b border-slate-100">
              <tr>
                ...
                {isAdmin && <th className="text-left p-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">Skill</th>}
                  ...
                  {isAdmin && (
                    <td className="p-4">
                      <button
                        ...
                        className={`flex items-center gap-1 px-2 py-1 rounded-md text-xs font-bold transition-colors ${player.feesPaid ? 'bg-emerald-100 text-emerald-700 hover:bg-emerald-200' : 'bg-red-50 text-red-500 hover:bg-red-100'
                          }`}
